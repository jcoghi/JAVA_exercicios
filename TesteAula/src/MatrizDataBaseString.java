...
public class MatrizDataBaseString {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < vendas.length; i++) {
            if (vendas[i] < meta) {
                System.out.println(vendedores[i] + " não atingiu a meta");
            } else {
                if (vendas[i] >= meta * 1.1) {
                    float salario = Math.round((float) salarios[i] * 110) / 100;
                    System.out.printf("%s ganhará R$ %.2f \n", vendedores[i], salario);
                } else {
                    System.out.println(vendedores[i] + " bateu a meta, mas não tem bonus de salário");
                }
            }
        }
